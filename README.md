...
| `.agents/skills/`                        | Playbooks: backlog, branch, changelog, changeset, documentation, humanizer, issue, jsdoc, pr, trim                                           | On demand, when a task matches the skill                         |
...
| `tools/*/commands/trim`                  | `/trim` audits the current branch's changes for AI-generated code smell (over-engineering and style tells) and applies only what you confirm | When you type the command                                        |
