...
Gemini CLI extension: its commands are TOML, and because Gemini has no
on-demand skill loading, `tools/gemini/GEMINI.md` inlines the conventions and is
generated by `scripts/agentFiles.ts`. Edit the rules, then run
`pnpm agent-files --write`. Never edit the generated file. `tools/opencode/` and
