...
- [issue](.agents/skills/issue/SKILL.md) - Open a GitHub issue with its sidebar filled in, so the labels, the type, and the Priority, Start date, Target date, and Effort fields are set rather than left empty. Use when asked to file an issue, turn a bug report into one, or triage an issue that has no fields.
