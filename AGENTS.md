...
- [issue](.agents/skills/issue/SKILL.md) - Open a GitHub issue with its sidebar filled in, so the labels, the type, and the Priority, Effort, Start date, and Target date fields are set rather than left empty. Use when filing an issue, turning a report into one, or triaging an issue whose fields are empty.
